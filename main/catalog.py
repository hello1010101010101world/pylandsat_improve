...
class Catalog:
    ...
    def search(self, begin, end, path=None, row=None, geom=None,
        ...
        if path and row:
            scenes_total=[]
            for (i,j) in zip(path,row):
                i_list, j_list = _to_list(i), _to_list(j)
                scenes = self.db.query(
                    query=queries.CATALOG_SEARCH_PATHROW,
                    params=(i_list, j_list , begin, end, maxcloud, sensors, tiers))
                scenes_total.extend(scenes)
        elif geom:
            scenes_total = self.db.query(
        ...
        if slc:
            scenes_total = scenes_total.copy()
            scenes_total = [scene for scene in scenes_total if _slc_on(scene)]
